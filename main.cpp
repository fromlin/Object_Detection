...
#define WIDTH		848 //1920
#define HEIGHT		480//1080
#define D_WIDTH		848//1280
#define	D_HEIGHT	480//720
#define FPS			60//30
#define D_FPS		90//30
...
int main(int argc, char * argv[]) try
{
	...
	//colorizer color_map;The imported project "C:\Microsoft.CSharp.targets" was not found.

	config d_cfg;	// , d_cfg;
	//cfg.enable_stream(RS2_STREAM_COLOR, WIDTH, HEIGHT, RS2_FORMAT_BGR8, FPS);
	d_cfg.enable_stream(RS2_STREAM_DEPTH, D_WIDTH, D_HEIGHT, RS2_FORMAT_Z16, D_FPS);

	// Start streaming with own configuration
	pipeline d_pipe;	// , d_pipe;
	//pipe.start(cfg);
	d_pipe.start(d_cfg);
	...
	while (waitKey(1) < 0 && getWindowProperty(d_cam, WND_PROP_FULLSCREEN) >= 0) //getWindowProperty(d_cam, WND_PROP_FULLSCREEN) >= 0 && 
	{
		//frameset data = pipe.wait_for_frames();
		//frame color = data.get_color_frame();
		frameset d_data = d_pipe.wait_for_frames();
		frame depth = d_data.get_depth_frame().apply_filter(color_map);
		...
		Mat d_image(Size(d_w, d_h), CV_8UC3, (void*)depth.get_data(), Mat::AUTO_STEP);
